...
describe('Scoring Engine', () => {
  ...
  it('score les stations correctement', () => {
    const scored = scoreStations(mockStations, FuelType.REGULAR);
    expect(scored.length).toBe(2);
    // Station B (moins chère) devrait avoir le meilleur score
  ...
  it('exclut les stations sans prix valide', () => {
    ...
    const scored = scoreStations(stationsNoPrices, FuelType.REGULAR);
    expect(scored.length).toBe(0);
  });

  it('getTopStations retourne le N supérieur', () => {
    let scored = scoreStations(mockStations, FuelType.REGULAR);
